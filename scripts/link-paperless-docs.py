...
def query(
    ...
    log = logger.new(url=url)
    with httpx.Client(cert=connection.cert) as client:
        log.debug("Querying Paperless API", url=url, params=params)
        ...
        if res.status_code != 200:
            ...
            log.debug("Response", response=res.text)
...
def link_documents_to_dst(
    ...
    if not dst_path.exists():
        if not dry_run:
            ...
            click.echo(f"Would create directory: {dst_path}")
        ...
        with bound_contextvars(doc_id=doc.id, doc_title=doc.title):
            ...
            if not dst_file_path.parent.exists():
                if not dry_run:
                    ...
                    click.echo(f"Would create directory: {dst_file_path.parent}")
...
def remove_stale_files_and_empty_dirs(
    ...
    logger.debug("Removing stale files and empty directories")

    # Remove stale files
    stale_files = existing_files - linked_files
    for stale_file in stale_files:
        if dry_run:
            click.echo(f"Would remove stale file: {stale_file}")
        else:
    ...
    for dir_path in sorted(dst_path.rglob("*"), reverse=True):
        if dir_path.is_dir() and not any(dir_path.iterdir()) and dir_path != dst_path:
            if dry_run:
                click.echo(f"Would remove empty directory: {dir_path}")
            else:
...
def link_file(
    ...
    if dry_run:
        click.echo(f"Would link {src_file_path} to {dst_file_path}")
    else:
        ...
        elif dst_file_path.stat().st_ino != src_file_path.stat().st_ino:
            logger.warning(
                "Destination file exists and points to a different inode",
                src_file_path=src_file_path,
                dst_file_path=dst_file_path,
            )
...
def main(
    ...
    try:
        ...
        click.echo(
            f"Error: Unable to connect to the Paperless API. Please check your network connection and API URL.\n{str(e)}",
            err=True,
        )
        raise click.Abort()
    except httpx.HTTPStatusError as e:
        click.echo(
            f"Error: HTTP error occurred while connecting to the Paperless API.\n{str(e)}",
            err=True,
        )
        raise click.Abort()
